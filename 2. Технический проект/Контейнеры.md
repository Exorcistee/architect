...
System_Boundary(platform, "Платформа для онлайн-консультаций") {
    ...
    System_Boundary(core_services, "Services") {
        ...
        Container(video_service, "Video Service", "NestJS + WebRTC", "Видео-конференции, сигналинг")
    }
    
    ContainerDb(postgres_db, "Основная БД", "PostgreSQL 15", "Пользователи, бронирования, платежи")
    ContainerDb(redis_cache, "Кэш и Pub/Sub", "Redis 7", "Сессии, кэш, WebSocket соединения")
...
api_gateway --> video_service : "WebSocket (видео сигналинг)"
...
video_service --> redis_cache : "Активные сессии"
chat_service --> redis_cache : "Pub/Sub для сообщений"
notification_service --> redis_cache : "WebSocket соединения"

booking_service --> message_queue : "AMQP (события бронирования)"
payment_service --> message_queue : "AMQP (события платежей)"
notification_service --> message_queue : "AMQP (отложенные уведомления)"
analytics_service --> message_queue : "AMQP (обработка метрик)"

video_service --> video_ext : "WebRTC/TURN протоколы"
